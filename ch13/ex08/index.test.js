...
describe('fetchFirstFileSize', () => {
    test('return filesize when is not empty', async () => {
        const path = 'ch13/ex01/';
        const expectedSize = 134;
        const result = await fetchFirstFileSize(path);

        expect(result).toBe(expectedSize);
    });

    test('return null when is empty', async () => {
