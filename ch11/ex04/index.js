...
function arrayMultiply() {
  ...
  for (let i = 0; i < N; i++) { // lhsA の行数
    for (let j = 0; j < M; j++) { // rhsAの列数
      for (let k = 0; k < K; k++) { // lhsAの列数/rhsAの行数
        resultA[M * i + j] += lhsA[i * K + k] * rhsA[k * M + j];
      }
    }
  }
}
...
function typedArrayMultiply() {
  ...
  for (let i = 0; i < N; i++) { // lhsB の行数
    for (let j = 0; j < M; j++) { // rhsB の列数
      for (let k = 0; k < K; k++) { // lhsB の列数/rhsB の行数
